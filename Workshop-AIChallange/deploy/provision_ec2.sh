...
SG_NAME="${SG_NAME:-workshop-ai-challenge-sg}"
...
if [ -z "$SG_ID" ] || [ "$SG_ID" = "None" ]; then
  SG_ID=$(aws ec2 create-security-group \
    --group-name "$SG_NAME" --description "Workshop AI Challenge app" \
...
INSTANCE_ID=$(aws ec2 run-instances \
  ...
  --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=workshop-ai-challenge}]' \
  --region "$AWS_REGION" \
